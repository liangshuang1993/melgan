...
from mel2wav.dataloader import AudioDataset 
...
def main():
    ...
    test_set = AudioDataset(
        Path(args.data_path) / "val.txt",
        22050 * 4,
        sampling_rate=22050,
        augment=False,
    )

    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=8)
    ...
    test_voc = []
    test_audio = []
    for i, (s_t, x_t) in enumerate(test_loader):
        x_t = x_t.cuda()
        s_t = s_t.cuda()
    ...
    for epoch in range(1, args.epochs + 1):
        for iterno, (s_t, x_t) in enumerate(train_loader):
            x_t = x_t.cuda()
            s_t = s_t.cuda()
            x_pred_t = netG(s_t)
